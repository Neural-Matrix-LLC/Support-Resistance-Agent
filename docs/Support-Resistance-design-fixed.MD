<!--
Formatting note: This document uses portable Markdown formatting. Math-heavy LaTeX display blocks have been converted to readable Markdown/Unicode so they render correctly in standard Markdown viewers.
-->
...
**P(rejection| P_t∈ R,X_t)**

For breakout:

**P(breakout| P_t∈ R,X_t)**

For breakdown:

**P(breakdown| P_t∈ S,X_t)**
...
**VWAP_t=fracΣ_i=1^tP_iV_i Σ_i=1^tV_i**
...
**C = C_structure +C_volume +C_VWAP +C_options +C_timeframe +C_momentum +C_orderflow +C_market**
...
**Y_reaction = fracExtreme_post-touch-Entry ATR_20**
...
**OBI= fracBidDepth-AskDepth BidDepth+AskDepth**
...
**L= λ_1L_touch +λ_2L_hold +λ_3L_break +λ_4L_quantile +λ_5L_reaction**
...
**Score_z= f( Technical, Volume, VWAP, Structure, Options, OrderFlow, Regime, Market, Sector, ML, DL, Events )**
...
**Confidence = f( ModelAgreement, Calibration, DataQuality, RegimeStability, HistoricalSampleSize, CrossMarketConsistency )**
...
**Prediction= Global+ MarketEffect+ SectorEffect**
...
**BS=frac1NΣ(p_i-y_i)^2**

### Log loss

**LL=-frac1NΣ[y_ilog p_i+(1-y_i)log(1-p_i)]**
...
**NetReturn= GrossReturn- Commission- Spread- Slippage- MarketImpact**
...
**Priority = P(touch) × P(reaction) × ExpectedMagnitude × Confidence**
...
**GlobalRepresentation + MarketEmbedding + ExchangeSpecificFeatures**
...
**Edge=P(win)× E(win) -P(loss)× E(loss)**
...
**Outtext-of-sample\ calibrated\ predictive\ value + economic\ value**
...
**Where are the statistically significant price zones?**

followed by:

**P(texttouch),\ P(hold),\ P(break),\ E[reaction]**

conditioned on:

**textprice structure +volume +VWAP +options +order flow +market regime +sector +events +text**
